...
//    return Mono.from(dsl.delete(GROUP).where(GROUP.ID.eq(gid)))
//        .thenMany(dsl.delete(GROUP_MEMBER).where(GROUP_MEMBER.GID.eq(gid)))
//        .then();
//  }
//
//  @Override
...
//    return Mono.from(dsl.selectFrom(GROUP).where(GROUP.ID.eq(id)))
...
//    return Mono.from(dsl.selectCount().from(GROUP).where(GROUP.ID.eq(gid)))
