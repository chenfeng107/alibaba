...
class HttpExceptionHandlerTest {
  ...
  void applyEnabledDebug() {
    ...
    // SPY
    var httpExceptionHandler = Mockito.spy(new HttpExceptionHandler());
  ...
  void applyNoDebug() {
    ...
    // SPY
    var httpExceptionHandler = Mockito.spy(new HttpExceptionHandler(false));
  ...
  void applyBizCodeException() {
    ...
    assertThat(statusCaptor.getValue()).isEqualTo(ex.getBizCode().getHttpStatus());

    var problem = problemCaptor.getValue();
    assertThat(problem).hasFieldOrPropertyWithValue("status", ex.getBizCode().getHttpStatus());
    // 开启调试模式下将 stacktrace 通过接口输出
